...
Nous avons utilisé les commandes suivante lors de ce tp : 
	mvn clean : nettoyer ce qui c'est passé avant
	mvn compile : voir les erreurs de compilations
	mvn package : fabrication d'une version executable
	java -jar target/tp_qualite-1.0.jar : observer la creation du fichier .jar et observer l'execution
	mvn site : creer une javadoc

Ces commandes nous ont dojnnc permis de decouvrir de nouvelles choses possibles à faire grace a github.